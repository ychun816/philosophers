...
/*   Updated: 2025/01/20 19:58:47 by yilin            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
 ...
 *
	-> "-1" means there is no "must eat" condition (unlimited eating until the simulation ends due to other factors,
	such as a philosopher's death).
 * table->feast_stop = false;
 * -> It acts as a flag to indicate whether the simulation has concluded.
 *
	-> The value starts as false because the simulation hasn't started yet and will remain false while philosophers are eating,
	thinking, and living.
 * -> It signals that the simulation should stop,
	likely because one of the philosophers has died (or another termination condition has been met).
 *
 */
 ...
 * - Fork mutexes (table->forks):
	...
	* Each fork is represented by a mutex to ensure that only one philosopher can "pick up" a fork at a time. Without these mutexes,
	multiple threads could access the same resource simultaneously,
	leading to undefined behavior.
 * - Print mutex (table->print_status_mutex):
 * Ensures that log messages (e.g.,
	a philosopher eating or dying) are printed sequentially without overlapping from multiple threads.
 * - Death mutex (table->death_mutex):

	* Protects access to shared data related to the simulation's termination conditions,
	like detecting a philosopher's death.
 *
 * @note
 * - Philosopher 1 uses:
 *   Left fork: forks[0]
